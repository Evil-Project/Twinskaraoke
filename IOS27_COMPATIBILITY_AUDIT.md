...
This pass vendored the MIT-licensed swift-spleeter 0.2.0 source with a compatibility patch: a `separateFile` entry point that ran the chunk loop and file writers in the caller's task, plus CPU-only execution for the model configuration and the task-local `MLTensor` policy on iOS 27. **That vendoring has been reverted.** Separation was broken on device against the patched copy, and the patch's benefit was speculative where its cost was not, so the app is back on the released `jiyimeta/swift-spleeter` 0.2.0 package and its streaming `separate(from:to:)` API, with the caller checking cancellation each chunk as it did before. The orphaned-producer window that `separateFile` was written to close is therefore open again: the writers run in a task the stream owns, so abandoning the stream does not by itself guarantee they have stopped touching the output files. `runSeparation2` still separates into a per-job temporary directory and only promotes the stems after a cancellation check, which bounds the consequence to discarded temporary files.

The reverted patch also pinned `MLModelConfiguration.computeUnits` and the task-local `MLTensor` compute policy to CPU-only on iOS 27, to avoid GPU/Neural Engine work across foreground-to-background transitions without a provisioning-dependent entitlement. Compute-unit selection is the framework default again. The underlying constraint is unchanged and still unmeasured: ordinary background audio does not grant unlimited CPU execution time, and iOS may suspend the app when playback stops, so foreground/background completion, battery use and memory during separation still need measuring on hardware. If that measurement shows a problem, the compute policy belongs in the app's own model configuration rather than in a fork of the dependency. [Background Inference entitlement](https://developer.apple.com/documentation/bundleresources/entitlements/com.apple.developer.background-tasks.continued-processing.inference), [MLTensor compute policy](https://developer.apple.com/documentation/coreml/mlcomputepolicy)
...
| CoreML/Accelerate/Spleeter | Model load, separation producer/consumer, trim reader/writer, cancellation and output publication | Released package restored after the vendored patch broke separation on device; cancellation is checked per chunk by the caller. Compute-unit policy is the framework default again. |
...
Latest release status is not an iOS 27 compatibility guarantee. The Spleeter source inspection above is more informative than its version number. All four dependencies are consumed as released packages; nothing is vendored. The deployment target remains iOS 26.5.
