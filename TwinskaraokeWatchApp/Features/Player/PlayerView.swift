...
struct PlayerView: View {
            ...
            GeometryReader { geo in
                ...
                ScrollView {
                    VStack(spacing: metrics.contentSpacing) {
                        ...
                        .accessibilityHint(primaryControlHint(for: song))
                        .accessibilityAction {
                            togglePlayPause()
                        }
                        ...
                        .accessibilityIdentifier("WatchPlayer.nowPlaying")
                        VStack(spacing: 1) {
                        ...
                        HStack(spacing: metrics.mainControlSpacing) {
                            ...
                            WatchPlayerIconButton(
                                systemName: primaryControlSystemName,
    ...
    private var primaryControlSystemName: String {
        if audioManager.isLoading { "stop.fill" } else if audioManager.isPlaying { "pause.fill" } else { "play.fill" }
    }

    private var primaryControlAccessibilityLabel: String {
        if audioManager.isLoading { "Cancel Loading" } else if audioManager.isPlaying { "Pause" } else { "Play" }
    }

    private func primaryControlHint(for song: Song) -> String {
        if audioManager.isLoading {
            "Cancels loading \(song.title)."
    ...
    private var queueAccessibilityValue: String {
        WatchQueuePresentation.accessibilityValue(
            upNextCount: audioManager.upNextSongs.count,
